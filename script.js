...
function renderCustomTechniqueCards() {
    ...
    document.querySelectorAll(".custom-card-wrapper").forEach(function(item) {
        item.remove();
    });

    getCustomTechniques().forEach(function(technique) {
...
function updateTechniqueDisplay() {
    ...
    items.forEach(function(item) {
        ...
        const matchesFilter = filterMode === "all" || filterMode === source;
    ...
    items.sort(function(a, b) {
        ...
        const orderA = Number(cardA.dataset.order || 0);
        const orderB = Number(cardB.dataset.order || 0);
...
function setupTechniqueForm() {
    ...
    form.addEventListener("submit", async function(event) {
        ...
        const newTechnique = {
            ...
            use: use,
            standard: standard,
            practice: practiceText ? practiceText.split("\n") : ["No practice steps added yet."],
            videoId: videoId
        };
        ...
        if (saveMessage) {
            saveMessage.textContent = "Saved! Your new technique was added.";
        }
...
function deleteCustomTechnique(id) {
    const updated = getCustomTechniques().filter(function(technique) {
...
function loadTechniqueDetail() {
    ...
    const technique = params.get("technique") || "slidehopping";
...
async function loadCustomTechniqueDetail() {
    ...
    const data = getCustomTechniques().find(function(item) {
...
function renderCustomChecklistItems() {
    ...
    document.querySelectorAll(".custom-practice-row").forEach(function(item) {
        item.remove();
    });
    ...
    getCustomTechniques().forEach(function(technique) {
        ...
        row.innerHTML =
            "<span class='difficulty-badge " + difficultyClass + "-badge'>" + technique.difficulty + "</span>" +
            "<span class='practice-number'></span>" +
            "<a href='detail.html?technique=" + technique.id + "' class='practice-name'>" + technique.title + "</a>" +
            "<span class='practice-standard'>" + (technique.standard || "Complete this technique consistently.") + "</span>" +
            "<label class='practice-check'><input type='checkbox'><button class='delete-practice-custom'>Delete</button></label>";

        row.querySelector(".delete-practice-custom").addEventListener("click", function(event) {
            event.preventDefault();
...
function renumberPracticeRows() {
    document.querySelectorAll("#practiceChecklist .practice-number").forEach(function(number, index) {
        number.textContent = index + 1;
    });
}

function setupChecklistListeners() {
    document.querySelectorAll("#practiceChecklist input[type='checkbox']").forEach(function(checkbox) {
...
function updateProgress() {
    ...
    rows.forEach(function(row) {
        const weight = getPracticeWeight(row.dataset.difficulty || "beginner");
    ...
    if (progressText) {
        progressText.textContent = "Progress: " + completed + " / " + rows.length + " completed";
    }
